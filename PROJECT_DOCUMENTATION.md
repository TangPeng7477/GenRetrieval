...
| 指标 | IandS（域 A） | VG（域 B） |
|---|---:|---:|
| 数据集 | Amazon Reviews 2023 · 官方 5core/timestamp 分片 | 同 |
| 商品数 | **25,847** | **25,612** |
| 用户数（有 train 历史） | **46,341** | **91,540** |
| 原始交互（去重后） | **412,947** | **814,586** |
| train / valid / test 样本 | **251,576 / 16,074 / 13,232** | **645,055 / 14,231 / 11,276** |
| 历史长度上限 | 20（train mean 4.8/p50 3；test mean 6.38/p50 5） | 20（train mean 6.22/p50 4；test mean 6.89/p50 5） |
| 图像覆盖率 | **99.91%**（URL 侧 100%） | **99.84%**（URL 侧 100%） |
| 时间切分 | train ≤2021-08-11 ｜ valid 2021-08~2022-07 ｜ test 2022-07~2023-08 ｜ **无泄漏（已验证）** | 同一边界（官方全局时间轴统一切分） |
...
| 切分 | leave-one-out（用户级，全局时间交错） | **官方全局时间切分（无泄漏）** | 更严格 |
| 类目数 | 1 | **2（独立成集并行）** | — |
...
> ⚠️ 这些数字来自 **2018 数据 + leave-one-out 切分**，与本项目的新数据**不可比**。本项目必须在新数据上重跑 V0 配置作为真正的 baseline（对应 R1）。
...
【数据】  Amazon 2023 I&S ｜ 25,847 items ｜ 46,341 users ｜ train/valid/test = 251,576/16,074/13,232
...
【V0 参照】SFT HR@10 0.093 → RL 0.109（+17%）｜ NDCG@10 0.060 → 0.074（+23%）｜官方 1.5B 0.154
          ⚠️ 2018 数据 + leave-one-out 口径，与本项目不可直接比
