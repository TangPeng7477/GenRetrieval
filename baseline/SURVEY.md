...
| 口径维度 | 论文常见设置 | 本项目设置（`baseline/README.md`） | 对数字的影响 |
|---|---|---|---|
| **数据集年份** | TIGER/LC-Rec/LETTER/EAGER/RPG/GRID = Amazon **2014 或 2018** | Amazon **2023**（官方 5core/timestamp 分片） | 不可比。2023 版类目更大、更稀疏、上新更多 |
| **切分方式** | leave-one-out（每用户留最后 1 条做 test） | **官方全局时间切分**（train ≤2021-08-11，test 2022-07~2023-09） | 本项目 test 是**纯未来**，且 **33% 的 test 目标商品在 train 中从未出现**（真冷启动）→ 显著更难 |
| **候选集** | 少数工作用 100 负采样 | **全库排序**（2.5 万物品） | 负采样数字系统性偏高，不可混用 |
| **K 值 / beam** | K=5,10；生成式 beam 常为 10~50 | K=1,5,10 + MRR；生成式 beam=20 | 生成式的 HR@10 **天花板由 beam 决定**，必须连 beam 一起报 |
| **是否屏蔽已交互** | LOO 下用全序列，天然屏蔽 | **屏蔽 history ∪ 完整训练序列 ∪ valid 目标**（见 `common/data.py`） | 不屏蔽会虚高；我们主动做严 |

⚠️ **最要命的一条**：LOO 的 test 目标在时间上可能落在 train 区间内部；本项目是纯外推。
所以即使我们把 SASRec 跑到与 CCFRec 一样的超参，**分数也应当低于它的 0.0412/0.0847**，
这不是实现错了，是任务更难。对外报数时必须写清口径。
...
#### ⑤ v2 → v1 的差额（实测已验证，2026-09-13）

| 域 | v1 timestamp test 数 | v2 LOO test 数 | 倍数 |
|---|---:|---:|---:|
| I&S | 13,232 | **50,985** | **3.85×** |
| VG | 11,276 | **94,762** | **8.4×** |
...
| **I&S test 样本数** | **50,985** | **50,985**（重跑后实测） | ✅ **一致** |
| **VG test 样本数** | **94,762** | **94,762**（重跑后实测） | ✅ **一致** |
