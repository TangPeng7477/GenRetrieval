...
| items | 25,847 | 25,611 |
| users | 46,341 | — |
| train / valid / test | 208,999 / 50,984 / 50,982 | — |
...
### 4.4b ⚠️ LCP / ratio 的两个读法陷阱（2026-09-13 双域实测后补）

**陷阱一：分母极小，ratio 会被随机基线抖动放大。**
随机对的 LCP 均值仅 **0.0045~0.0059**。VG 上 RQ-VAE 与 RQ-KMeans 的 `LCP(nn)` 只差 **0.4%**
（0.9680 vs 0.9717），但 `ratio` 差了 **26%**（163.6 vs 206.3）——差异全部来自分母。
**报 LCP 必须同时给 `nn_mean` 与 `ratio`。**

**陷阱二：跨域不可横比，甚至排序反转。**
IandS 上 RQ-VAE 的 LCP(nn) 是 0.9950 vs RQ-KMeans 0.8893（**RQ-VAE 大胜**）；
VG 上变成 0.9680 vs 0.9717（**RQ-KMeans 微胜**），而同期 RQ-KMeans 的 raw ICR 落后 8.6 个点。
原因：VG 的语义近邻对本来就落在同一个粗簇内（RQ-KMeans 的 top-5 邻居首 token 命中率也有 0.659），
**前缀结构在该域已饱和、失去区分度**。

→ **跨域选型主判据改用 raw ICR + 重建 R²**（两项两域排序一致）；
LCP / cohesion 只作**域内**结构参考。完整推导见 `SID_PIPELINE.md` §2.11。
...
1. **ICR 高 ≠ SID 好**：逐层 Sinkhorn 能把 ICR 做到 1.0，但 LCP 反而变差（前缀层级被抹平）。
   更强的反例：Sinkhorn 能靠 batch 分桶把**逐位相同的孪生 embedding** 拆到不同码
   （VG 实测 `sid_sk` ICR 0.9954 > 纯 argmin 上限 0.9936），但这些物品本就是同一商品的不同 listing
   ——**指标涨了，语义上什么都没发生**。
2. **改码比例与保真度要一起看**：唯一性是用 Δ_code 换来的，代价是 ΔMSE / LCP 下降。
3. **代理指标 ≠ 下游指标**：R@k、LCP、cohesion 都是秒级代理，最终仍要 HR@K / NDCG@K 收口。
4. **单域结论不能外推到第二个域**（2026-09-13 双域复验的教训）：
   LCP 族指标在 VG 上排序反转（§4.4b），"ICR 上限"的适用口径也被第二个域推翻。
   → **任何"选型判据"都应该在两个域上各跑一遍再写进文档**，否则只是单点观察。
...
### 5.6.3 🔴 VG 的 L0 死码：塌缩只发生在 warmup 结束那一轮，且**恢复程度由数据的有效秩决定**

**Q：I&S 用 init8192 把 L0 死码打到 0% 了，为什么 VG 上又出现 3.52%（9/256）？**

**A：同一个塌缩，只是 VG 没恢复完。而且"恢复得完不完整"取决于输入点云的有效维度。**

**先看逐 50 轮的 L0 用码数**（`train_metrics.json → curve[]`，只记 ep 1,50,100…）：
...
1. **塌缩时刻是 warmup 结束那一轮**（`warmup_epochs=50`，lr 正好到峰值 1e-3），
   两域同时跳水，`collision` 同时见顶（VG 0.1069 / IandS 0.0330）。
   机理同 §5.6：init pass 的 k-means 是在**随机 encoder** 输出上聚的，encoder 一学会，
   初始划分就作废 → 一批质心同时变空。
2. **VG 塌得更深（56 vs 122）且没恢复完**：到 5000 轮才 250，**全程从未回到 256**；
   I&S 在 ep1000 就满血。查死码时它还在缓慢爬升 —— 说明"死码"不是永久判决，
   但**爬出死亡区需要数据云重新扩张到质心附近**。
3. **VG 的 9 个死码是"孤岛"而不是"被挤死"**（`scripts/multimodal/probe_latent_rank.py` 实测）：
   它们恰好是"**离样本最远的质心**"榜的**第 248~256 名**（共 256 名）；
   死码到最近样本距离 **0.1737~0.1777**，而活码同一指标 **max 0.1129**——
   两群**零重叠**，间隙 1.54×。argmin 永远不选它们 → 收不到梯度 → 永久死。

**为什么会多出这 9 个孤岛？—— 输入空间有效秩差一个量级。**
同一份 1024 维融合向量的协方差谱：
...
| 前 32 PC 方差占比（=瓶颈宽 32） | 32.4% | **59.9%** |

VG 的 1024 维里**只有约 32 维在动**。把点云压到 32 维 latent、再用 **256 个 Voronoi 元胞**铺：
**有效维度越低，越多元胞落进"没有数据的空方向"**，少数元胞吞掉大部分质量（VG 头码 614 个物品，
I&S 最大 241），边缘元胞直接成孤岛。

**这条与"重建更容易"是同一件事的两面**：VG 前 32 PC 覆盖 59.9% 方差 ⟹ 32 维瓶颈够用 ⟹
R² 0.8691（I&S 只 0.6530，因为前 32 PC 仅 32.4%）。**"好压"和"会塌"同源。**

**处置（按性价比）**：① `--init_samples` 上调 / 换 seed；② diversity loss 或 dead-code reset
（§5.6 主表的"修复"路线）；③ **L0 码本 256 → 128**（VG 本来只用 247，顶层是过配的）。
**根本教训：码本大小应由数据的有效秩决定，不能照抄上游配方；单域结论不可外推。**
完整版见 `docs/SID_PIPELINE.md §2.8.2`，数据侧视角见 `docs/DATASET.md §8.6`。
...
→ `results/sid_e5000/{IandS,VG}/compare_rqkmeans.md`。同一 emb（gate_e60）、同一 eval_sid.py。
**两域都跑了**（VG 于 2026-09-13 作为跨域复验补跑）：

| 指标 | IandS RQ-VAE raw→sk | IandS RQ-KMeans raw→sk | VG RQ-VAE raw→sk | VG RQ-KMeans raw→sk |
|---|---|---|---|---|
| ICR | 0.9582 → 0.9997 | 0.8395 → 0.9996 | **0.9744 → 0.9954** | 0.8882 → 0.9959 |
| 最大冲突组 | 5 → 2 | 14 → 3 | 7 → 5 | 12 → 4 |
| 死码率 L0/L1/L2 | 0 / 0 / 0 | 0 / 0 / 0 | **3.52 / 0 / 0** | 0 / 0 / 0 |
| LCP(nn) | 0.9950 → 0.9383 | 0.8893 → 0.8240 | **0.9680 → 0.9427** | **0.9717 → 0.9160** |
| LCP ratio | 222.1 → 209.4 | 175.8 → 163.3 | **163.6 → 159.5** | **206.3 → 195.5** |
| prefix-1 内聚比 | 2.086 | 2.129 | 1.585 | 1.605 |
| 保真度 R² | 0.6530 → 0.6504（decoder 重建） | 0.4460 → 0.3797（直接量化） | **0.8691 → 0.8683** | 0.7770 → 0.7583 |
| Sinkhorn 改码比例 | 6.54% | **27.06%** | **3.93%** | 18.10% |
| 训练/导出耗时 | 3,644 s | **6 s** | 4,015 s | **5.5 s** |

结论（四条）：

1. **raw ICR 差距巨大（IandS 0.84 vs 0.96；VG 0.89 vs 0.97）**：beam=1 贪心残差量化在 1024 维高维空间里碰撞率高得多，
   这就是 TIGER/MQL4GRec 系都改用「低维 latent + 可学习码本 + 迭代训练」的动机——
   RQ-VAE 的 encoder 先把空间压到 32 维再量化，码本覆盖效率高一个量级。**两域一致。**
2. **Sinkhorn 之后唯一性打平，但 Sinkhorn 代价差了 4~5 倍**：
   rqkmeans 要改写 27.06%（IandS）/ 18.10%（VG）物品的码，RQ-VAE 只要 6.54% / 3.93%。
   代价小的直接好处是语义结构受损更轻——**两域一致。**
3. ⚠️ **LCP 这一族在第二个域上排序反转**：IandS 上 RQ-VAE 的 LCP(nn) 0.9950 完胜 rqkmeans 0.8893；
   VG 上变成 0.9680 vs 0.9717（rqkmeans 微胜），LCP ratio 也反转（163.6 vs 206.3）。
   但同期 rqkmeans 的 raw ICR 落后 8.6 个点、最大冲突组 12 vs 7、Sinkhorn 改码贵 4.6 倍。
   → **前缀结构在 VG 上已饱和、失去区分度**，详见 §4.4b。**这条是双域复验才暴露的，单域看不出来。**
4. **耗时差 600 倍**：6 s vs 3,644 s。如果只看 ICR，rqkmeans 是"免费午餐"；
   但叠加 raw ICR、保真度与 Sinkhorn 代价（R² 0.38~0.76 vs 0.65~0.87，口径差异见对比表注），
   RQ-VAE 的优势是系统性的。**维持决策：SID 默认 RQ-VAE(gate+init8192+5000 轮，导出 sid_raw)，
...
2. **消解不彻底，而且它"拆开"的正是本不该拆的**：sk 只能到 0.9997（IandS）/ **0.9954（VG）**，
   残留**逐份都是**逐位相同的重复 embedding——双域逐组账本：IandS 残余 7 份 / VG 残余 119 份，
   100% 落在孪生组内，非孪生碰撞被清得一份不剩（逐组账本见 `DATASET.md` §8.5 / `SID_PIPELINE.md` §3.5）。
   代价是改写 6.54%（IandS）/ **3.93%**（VG）物品的码、LCP −12.7 / −4.1、R² −0.0026 / −0.0008。
   更关键的是**它把 85.0%（IandS）/ 74.1%（VG）的孪生物品改到了别的码上**——
   这些物品本来就是同一商品的不同 listing（连主图 URL 都相同），拆开它们在语义上没有任何对应物，
   只让 ICR 变好看。**这是"ICR 不该当黄金标准"最直接的证据**（详见 `SID_PIPELINE.md` §3.5）。
...
| 版本 | 量化方式 | 用途 |
|---|---|---|
| **`sid_raw`（默认交付）** | `get_indices(use_sk=False)` 纯 argmin | **保真度与语义结构最好**（I&S LCP 222.1 / R² 0.6530；**VG LCP 163.6 / R² 0.8691**）；碰撞 = 语义桶，交给下游排序消歧 |
| `sid_sk`（存档对照） | 对**冲突物品**施加最后一层 Sinkhorn，迭代至无碰撞 | 唯一性最高（I&S ICR 0.9997 / **VG 0.9954**）；改码 6.54% / **3.93%**，LCP/R² 略降 |
