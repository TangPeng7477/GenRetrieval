...
| 模型 | HR@10 | 来源 |
|---|---:|---|
| **未训练 Qwen3-0.6B**（beam=20，`chatml`） | **0.0000** | §3.5.3 实测 **2000 条**（95% 上界 `3/2000` = 0.15%）；本节另有 300 条一次 |
...
#### 3.5.3 `[实测]` 本地跑"全量"要多久，以及为什么 batch 压不上去（2026-09-18）

起因："之前全量评估没这么久"。这里有个**同名不同物**的坑，先分清楚：

| 叫法 | 实际在做什么 | 全量（~5.1 万条）|
|---|---|---|
| **训练内 eval** | `Trainer.evaluate()`：teacher-forcing **一次前向**算 loss | **82 min**（`0.096 s/条`）→ §3.8 |
| **生成式 eval** | `evaluate.py`：自回归 + **beam=20** 约束解码 | **8~9 h**（`0.56~0.63 s/条`） |

两者差约 **7 倍** —— 生成式每条都要跑 20 个 beam × 约 5 步。"82 min"属于**前者**。

`[实测]` 生成式本地标定（3050Ti 4GB · beam=20 · `max_new_tokens=16` · 未训练基座 · IandS）：

| batch | 序列数 = batch×beam | 单批耗时 | s/条 | 外推全量 50,982 条 |
|---:|---:|---:|---:|---:|
| 2 | 40 | 1.13 s | **0.563** | **8.0 h** |
| 4 | 80 | ~2.5 s | **0.625** | **8.9 h** |
| 8 | 160 | **28 s** ✗ | 3.5 | 已中止（50 h+） |

🔴 **batch=8 的单批 28 s 比线性外推差 5.6 倍** ⟹ **落共享内存换页**：峰值超过 4 GB 物理显存，
Windows WDDM 允许超额分配 → "不 OOM 但奇慢"。与训练侧记的是**同一个机制**（§3.10）。
⟹ **本地 eval 的 batch×beam 上限约 80 条序列**（batch4 × beam20）；再往上不是变快，是变慢。

**历史对照**：2026-09-16 那次 300 条 / beam=20 跑了 `10:14`（§3.5）→ **2.05 s/条**，
按它外推全量要 **29 h**。现在 0.56~0.63 s/条是**快了约 3 倍**，不是变慢。
（另：`SFT_EVAL_RESULTS.md` 里 `IandS-untrained` 那行「耗时」是 `—`，因为它跑在计时上线之前。）

**结论**：4 GB 卡上本地全量生成式 eval 的现实代价就是 8~9 h，没有更快的配置 ——
batch 加不上去（换页）、beam 是 `HR@K` 硬上限不能压、评估协议里也没有"不算生成"的路径。
要省时间只有两条路：**抽样**（0 命中时 95% 置信上界 ≈ `3/N`：N=2,000 → 0.15%，N=50,982 → 0.006%）
或**上云**。

`[实测]` 2026-09-18 按上表选**抽样 2000 条**（batch=2 / beam=20 / chatml）真跑了一遍：

| 项 | 实测 |
|---|---|
| `eval_seconds` | **1187 s = 19m47s** → **594 ms/条**（与标定 563 ms/条 吻合，误差 5%） |
| GPU | 利用率 **100%**、显存 3412/4096 MiB（batch=2 已把卡喂满，不是"没喂饱"） |
| `HR@K` / `NDCG@K`（K=1/3/5/10/20） | 全 **0.0000** |
| `CC`（`n_generated_not_in_item_dict`） | **0** |
| 40,000 个候选的合法 SID 形态 | **40000/40000 = 100%** |
| 真值闭环（`verify_eval_chain.py --skip-a`） | `output`/`history_str` **0/2000 不一致** |

⟹ 抽样 2000 条时，`HR=0` 的 95% 置信上界 = `3/2000` = **0.15%**（全量 50,982 条才 0.006%）。
产物 `results/sft/IandS-untrained-chatml/`（已进 `SFT_EVAL_RESULTS.md`；`prompt_format=chatml` 记在 meta 里）。
