...
#### ⚠️ 补充实测（2026-09-15）：量化真正面对的是 latent，不是输入

上面的推理用的是**输入空间**（1024 维）的 PR，但 Voronoi 划分实际发生在 **latent 空间**。
补测两域谱（`probe_latent_rank.py`，同一次 forward）：

| 空间 | 域 | dim | 有效秩 PR | PC@50% | 前 8 PC 方差 |
|---|---|---:|---:|---:|---:|
| 输入（gate 融合输出） | IandS | 1024 | **109.2** | 111 | 20.7% |
| 输入（gate 融合输出） | VG | 1024 | **32.0** | 17 | 38.3% |
| **latent（量化真正面对的）** | IandS | 32 | **6.5** | 3 | 94.5% |
| **latent（量化真正面对的）** | VG | 32 | **5.4** | 2 | 94.7% |

两条读数：

1. **latent 有效秩两域只差 20%（6.5 vs 5.4），而输入 PR 差 3.4 倍（109 vs 32）。**
   encoder 把输入「归一化压缩」到近乎同构的低维结构 ⟹ **输入 PR 只是 latent 几何的弱代理**，
   不能当作死码的因果量直接引用。
2. **256 个 L0 元胞铺在一个有效维度仅 ~6 的点云上** —— 这种「容量/维度失配」是**两域共有**的，
   不是 VG 独有。VG 多出来的 9 个孤岛更可能与**点云尺度**（latent `trace(var)` 0.1268 vs IandS 0.2170、
   L0 残差 0.1085 vs 0.1518）和 **init pass 的铺开范围**有关。
   ⚠️ **精确归因待定**，此处只记实测，不写成已定论。

**同源补充：VG 的 latent 集中在融合阶段就已形成，不是输入带来的。**
四融合模式在**同一份输入**上的输出几何（`scripts/multimodal/probe_fusion_rank.py`，零训练）：

| 融合模式 | 监督信号 | IandS PR / 平均成对余弦 | VG PR / 平均成对余弦 |
|---|---|---:|---:|
| `text`（纯文本） | — | 87.2 / 0.396 | 73.2 / 0.488 |
| `concat`（PCA + L2n） | 无（无监督线性） | 110.8 / 0.000 | 73.6 / 0.000 |
| `mlp` | 共现 InfoNCE | 35.6 / 0.399 | 21.6 / 0.507 |
| **`gate`（定版）** | 共现 InfoNCE | **109.2** / 0.182 | **32.0** / 0.451 |

**同一份输入**下 `concat` 与 `gate` 的 VG PR 差 **2.3 倍**（73.6 → 32.0）、
IandS 上却基本持平（110.8 vs 109.2）⟹ **VG 的表示集中由监督融合目标造成**（VG 共现监督更密），
不是输入端编码器的毛病。`mlp` 两域都塌（35.6 / 21.6）—— 同为监督融合，门控结构保住了各向异性。

> 🔴 **这条对 V4 消融的直接影响**：把「提升输入各向异性」当作治死码的手段（`UPGRADE_PLAN §4.6.3` 的 V4-H）
> **靶子对不上** —— 输入 PR 从 32 提到 109 也不会让 latent PR 从 5.4 变成 6.5（IandS 就是 109 → 6.5 的现成反例）。
> 治 latent 几何要动**量化器/码本**（V4-M）或**训练目标**（V4-K），不是动输入预处理。
...
| 9 | **VG 的 L0 死码 3.52%**（9 个码弃用）未做机制验证 | 未触发预设逃逸条件（>5%）故未补跑 VG 的 init 消融；且"L0 死码必须为 0"这条判据本身来自单域经验，通用性存疑。**替代做法已有三家工业界先例**——PLUM 的**多分辨率码本**（`2048/2^(level-1)` 逐层递减）、OneSearch 第三层 **balanced k-means 防塌缩**、OneMall 的 **FSQ 收尾**（冲突率 36%→11%），合起来即 `UPGRADE_PLAN §4.6.4` 的 **V4-M** |
...
| 13 | **双塔的“异空间”性质未做替换消融** | 文本 `Qwen3-Embedding-0.6B`（1024 维）与图像 `SigLIP-base-224`（768 维）分属两个互不相通的嵌入空间，跨模态点积无定义、须绕道岭回归学线性映射（`fuse_embeddings.py::cross_modal_align` 注释原文：“点积没有定义”）；同为 Qwen 家族、提供**统一空间**的 `Qwen3-VL-Embedding`（2026-01 发布）未做替换对比。与 #7（域适应）、#8（编码器选型）同源但独立：**#7/#8 问“编码器够不够好”，本条问“两条腿是否本就该长在同一个空间里”**。待办 V4-A/B/C/D（换模型）与 V4-E/F/G（不改主干）及判据见 `UPGRADE_PLAN §4.6`／`§4.6.2`；「图文对齐」的四条技术路线文献地图（CLIP 双塔 / VLM 统一空间 / frozen-tower / 量化前对齐，含 2026-01~08 新工作）见 `UPGRADE_PLAN §4.6.1`。**第三条路（两路向量都不动、只学映射）的方法谱系与双域实测见 `§4.6.3`**——实测显示 `PCA 降维 + 两侧白化` 可把跨模态 R@10 从 I&S 0.5797 / VG 0.6100 提到 **0.8070 / 0.8758（+39% / +44%）**，且 `+0` 成本；但**该收益尚未验证对 SID 的传导**（待办 V4-H），**在 V4-H 出结果前不得当结论引用** |
| 14 | **图文接入 SID 的方式只做了 1 种（R1 + R4 最弱一档）** | 我们只有 `gate` 融合这一条（= **R1 早期融合**：量化前融成一个向量；其训练目标含**共现对比**，故同时落在 **R4 最弱一档**）。文献里另有三族未试：**R2 晚期融合**（各自量化再拼 token 序列：MQL4GRec 共享码本 / MSCGRec）；**R3 跨模态量化**（量化过程本身被跨模态监督：MACRec 双模态伪标签对比 / MSCGRec 的 RQ-DINO）；**R4 协同信号入模**（PLUM 共现对比正则 / OneMall 的 FSQ 收尾 / MSCGRec 把 SASRec embedding **单独 RQ**）。旁支 `Text-as-Vision`（把商品描述渲染成图再 OCR 编码，绕开 modality gap）在 **Amazon Instruments/Scientific（与本项目 I&S 直接重叠）** 上报告单模态 NDCG@5 **+8.06%**、早融合 **+13%以上**。🔴 但须同时记住反直觉证据：OneSearch-V2 在电商搜索上实测**单模态文本优于多模态、且「各自编码再拼接」最差**（详见 `UPGRADE_PLAN §4.6.4(b)`）——**融合增益不是给定的**。⚠️ **2026-09-15 修正**：本条原写「我方 V0' 与 `gate` 打得有来有回」，**有误**。我们自己的实跑是**融合显著胜出**——I&S 上 `text` → `gate__init8192`：ICR **0.8383 → 0.9582**、L0 死码 **65.5% → 0%**、LCP ratio **117.5 → 222.1**、内聚比 **1.548 → 2.086**、改码比例 **23.40% → 6.54%**；下游共现召回 holdout R@10 **0.0830 → 0.1100（+33%）**，VG 同向 **0.1497 → 0.2113（+41%）**。文献的负面结论对应的是我们的 **`concat`（PCA、无监督、线性）只比纯文本 +6%** 那一档（I&S 实为 0.0883 vs 0.0830，**我们复现了它**），**不是 `gate`**。原因：`mlp`/`gate` 是用 **72 万共现对监督训练**的，**增益来自共现监督、不是「接了图像」**（与 PLUM「共现对比项单项贡献最大」同机理，只是我们把监督放在融合阶段而非 RQ 损失里）。因此本条标题实为「**只做了 R1 + R4 最弱一档**」——**R0 已有对照**（`--mode text` 纯透传、不含图像、不训练 + V0' 锚点；⚠️ VG 域只有融合层代理数字、**无端到端 SID**），**R2/R3 全空白**，R4 的后三档（辅助任务 / 当第四模态 / STE 反传）未试。逐格状态表与「**R0 为何替代不了 R2（V4-P）与 V4-O**」的论证见 `UPGRADE_PLAN §4.6.6(f)`。五族路线图、各族实测数字、工业界七条共性、由本节新增的消融 **V4-K / V4-L / V4-M** 见 `UPGRADE_PLAN §4.6.4` |
