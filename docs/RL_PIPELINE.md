...
⚠️ **"每候选命中率"这个数暂不引用** —— `reward` 的分母尚未核定，见 §6.10 ⑻ 末。
...
⟹ 它只改变**命中组内部的相对奖励**（把正确答案抬到组内最高），对那 76% 零 advantage 的窗口毫无作用。
...
🔴 **仍未定：`reward` 的分母是 32 还是 128。** 取值网格是 **`k/32`**，而按代码读，一个优化步应含
`TRAIN_BATCH_SIZE 4 × NUM_GENERATIONS 4 × GRAD_ACC_STEPS 8` = **128 条候选**
（`_metrics["reward"]` 每 micro-batch append 一次，`:1037`；`log()` 后 `clear()`，`:1138`）
⟹ 网格本该是 `1/128`。**观测到的 `1/32` 与"8 次 append × 16 条候选"差 4 倍**，说明我的 batch 记账
还有一环没对上。**⟹ 核清前只引用「24% 的优化步有信号」这个比例（它对分母不敏感），
不要引用任何"每候选命中率"。**

判决手段（~3 min；压到单 micro-batch，网格就直接暴露单位）：

```bash
MODEL_PATH=outputs/IandS-all/final_checkpoint USE_LORA=False RUN_TAG=rlprobe \
MAX_STEPS=40 GRAD_ACC_STEPS=1 TRAIN_BATCH_SIZE=4 \
TEST_DURING_TRAINING=False EVAL_STEP=999 SAVE_STEPS=999 bash rl_run0.sh
grep -o "'reward': [0-9.]*" logs/rl/IandS-rlprobe/rl.log | sort | uniq -c
```

⟹ 网格 `1/16` = "4 prompt × 4 gen / micro-batch"（我最初的记账）；`1/4` = 每窗口只 1 个 prompt；
`1/32` = 每窗口 32 条候选。三者都能解释旧数据，但只有一种是真口径。
