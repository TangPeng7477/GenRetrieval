...
🔴 **但实测步速有 ±3× 的环境抖动 —— 调 batch 这件事先按停**：**同一配置**（B=4 / GA=8 / 21,395 行 /
`total=2675`）两次 run 的实测是——
`IandS-u5k` 旧 run（pid 25867）**3.17 s/step**（`999/2675 [52:57<1:28:39, 3.17s/it]`，稳定 999 步）；
同配置新 run（pid 67881）**1.07 s/step**（`104/2675 [01:52<45:57, 1.07s/it]`）。
`[已核]` 两者代码路径与工作量一致（无 torch.compile 痕迹、loss 字段同构、`completion_length` 均 5.0）
⟹ **3× 差额只能归给机器/环境（争用、功耗态、邻居租户），不是配置**。
⟹ `3.17` 与 `1.07` **都不许当常数**，`< 3.17 才算赢` 这类判据**作废**；成本必须**当场测 s/it**。
（连带：本仓"全量 1 epoch ≈ 30 h"是拿 3.17 外推的，1.07 下只有 ≈10 h ⟹ **需重测**，别据此排计划。）
...
步速 = 3.17 s/step（4090D，全参 GRPO + grad_ckpt + TEST_DURING_TRAINING=False；
       ...
       🔴 该值是 `IandS-u5k` 旧 run 实测；**同配置新 run 实测 1.07 s/step（差 3×，环境抖动）**
       ⟹ 下式只作量级参考，正式预算必须当场测 s/it。
⟹ 1 epoch ≈ 30 h，2 epoch ≈ 60 h（按 3.17；按 1.07 则 ≈10 h / 20 h，区间 **10–30 h**）
```
...
| 相当于多少行 | `MAX_STEPS` | 训练时长（见下注） | 其中有梯度的步（24%） |
|---:|---:|---|---:|
| 5,000 | 625 | ~33 min / **~11 min** | ~150 |
| 15,000 | 1,875 | ~1.7 h / **~34 min** | ~450 |
| 30,000 | 3,750 | ~3.3 h / **~1.1 h** | ~900 |

⚠️ **时长是两个实测步速下的区间**：粗体按新 run 实测 **1.07 s/步**、斜体前按旧 run 实测 **3.17 s/步**
（同配置差 3×，环境抖动，见 §6.8③）⟹ **只作量级参考，正式预算当场测 `s/it`**。
...
train ∩ test 用户 = 50,982 ；seed=42 抽 5,000 个
  ...
  训练集 = 20,418(T1) + 977(T3) = 21,395 行 ⟹ **2,675 步/epoch**（= ceil(21,395/8)）
  `[实测]` 新 run **1.07 s/step** ⟹ **≈48 min**（旧同配置 run 3.17 s/step ⟹ ≈2.4 h；步速 ±3× 抖动）
  ⚠️ **"5,000 用户"只让 test/valid 各 = 5,000 行（恰 1.00 行/用户）；train 侧是 4.08 行/用户 = 20,418 行**
     —— LOO 下每个用户有多个"前缀 → 下一条"的监督对，**一行 ≠ 一个用户**。
```
...
**✅ 阶段 0 判决（2026-09-25，`EXP_ID=IandS-u5k`：T1+T3、1 epoch、2,675 步、train 47 min、eval 19 min）**

| K | SFT 锚点 `IandS-all` | RL 后 `IandS-u5k` | Δ |
|---|---:|---:|---:|
| HR@1 | 0.0046 | **0.0056** | +22%（23→28 个命中，噪声级） |
| HR@3 | 0.0144 | 0.0144 | 0 |
| HR@5 | 0.0194 | 0.0188 | −3% |
| **HR@10** | **0.0342** | **0.0330** | **−0.0012 ≈ 0.5σ** |
| HR@20 | 0.0552 | 0.0542 | −3% |
| HR@50 | 0.0862 | 0.0850 | −2% |

**判决：持平，无信号。** 结果 `results/sft/IandS-u5k/eval_IandS_beam50_u5k.metrics.json`
（TEST_TAG 生效，没有顶掉锚点那份）。

**为什么没动（证据在训练日志里）**：rule 奖励 = **精确字符串匹配** ⟹ 信号极稀疏——
绝大部分步 `reward=0.0`、`reward_std=0` ⟹ 组内无方差 ⟹ **advantage=0 ⟹ 零梯度**
（`grad_norm` 多数 ~1e-4，偶尔才见 0.9–2.6 的真梯度）。策略确实在动（末端 `kl` 从 ~4e-4 涨到 0.04），
但**没有朝有用的方向动** —— 稀疏噪声推不动排序质量。
（口径：`reward=1/32` = 8 个 prompt 里恰 1 条命中；有信号步 ~13%，与 §6.8② 的 17%/24% 同量级。
另外这次训练用户 ≡ 评估用户（交集 100%），仍无增益 ⟹ 更坐实是**信号密度**问题，不是覆盖问题。）

**⟹ 下一步的杠杆是「奖励密度」，不是「再跑一遍同款」**：
1. **部分信用奖励**（推荐）：首位 SID 对 0.3 / 前两位对 0.6 / 全对 1.0 —— 把 ~0.5% 命中率的 0/1 信号
   变成几乎每步都有梯度；只改 reward 函数一处。
   **判据（起跑 2 min 即可看）**：`reward` 非零步占比应从 ~13% 升到 >50%。
2. `NUM_GENERATIONS` 4→8：组内含正例的概率近似翻倍 ⟹ 有梯度的组翻倍；代价是每步序列数翻倍。
3. 加到 2 epoch：有信号步只是翻倍，预期最弱。

**✅ 方案 1 已实现（2026-09-25，`reward_type="partial"`，新增类型、`rule` 原样保留可复现）**
`rl.py` 三处：模块级 `sid_partial_credit()`（档位 全对 1.0 / 前两位 0.6 / 仅首位 0.3 / 其余 0.0；
正则 `fullmatch` 三段 SID，**格式非法一律 0**，最坏退化为旧口径）；`partial_reward()`（同一闭包，
复用 `prompt2history`/`history2target`）；分支 + **显式 `else: raise`**（`REWARD_TYPE` 打错字当场报）。
`rule_reward` 语义未动。
`[实测]` 9 类判据全过：全对/带引号/前两位/仅首位/全错/缺第三位/乱码/空串/**前导零**
（`<a_01>` 与 `<a_1>` 是**不同 token**，必须 0 分）。`py_compile` ✓。
