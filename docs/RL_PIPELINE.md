...
| `IandS-all` 的现有锚点 | `HR@10` |
|---|---:|
| 全量 `n=50,982` | **0.0356** |
| `n=100` | **0.0200** ⟹ 比全量低 **44%** |
| `n=5,000` | **缺，需补** |

⟹ **`n=100` 与全量差 44%，就是"不同 N 不可比"的实证**。所以开快速迭代前**先补一次同口径锚点**
（一次性 ~16 min，之后所有迭代复用）：

```bash
MODEL_PATH=outputs/IandS-all/final_checkpoint MAX_SAMPLES=5000 BATCH_SIZE=12 bash evaluate_run0.sh
```

此后每个 fast run 都用**同一个** `MAX_SAMPLES=5000` 评 ⟹ 两次评估落在**完全相同的 5,000 行**上
（`random.seed(42)` 固定）⟹ 是**行级配对比较**，实际判别力优于表里的边际 ±7%。

⚠️ **不需要"按用户对齐"**：训练前缀与评估子集都是随机子集 ⟹ 用户分布本就匹配；行级配对已比用户级配对更紧。
但有一个**诚实的边界**：训练前缀 ≈ `15,000 / 4.1 ≈ 3,700` 个用户，评估 5,000 行 = 5,000 个用户
