...
未进 RQ 损失）；**R0 已占**（`--mode text` 纯文本对照 + V0' 锚点，⚠️ VG 域只有融合层代理数字），
**R2/R3 全空白**，R4 的后三档（辅助任务 / 当第四模态 / STE 反传）未试。
> 逐格状态与「R0 为何替代不了 V4-P / V4-O」的论证见 `§4.6.6(f)`。
...
> ⚠️ **先把「哪一格是空的」说准（2026-09-16 收紧措辞）**：我方**已经有纯文本对照**，而且有两份 ——
> ① `fuse_embeddings.py --mode text`（实现就是 `save(l2n(t_emb))`：**纯透传、不训练、不含图像**）
> 配完整 RQ-VAE 三件套（`results/sid_e5000/IandS/compare.md`：ICR **0.8383** / 第 0 层死码 **65.5%** / LCP ratio 117.5 / R² 0.8698）；
> ② V0' 锚点（MiniOneRec 纯文本）。**但它占的是 R0 格，不是 R2 格。**
>
> | 格 | 路线 | 我方状态 |
> |---|---|---|
> | **R0 纯文本** | 只有文本塔 | ✅ **已占**（`--mode text` + V0' 锚点）⚠️ 但 **VG 域没有端到端 SID**，只有融合层代理数字 |
> | **R1 量化前融合** | 图文先融成一个向量再量化 | ✅ **已占**（`gate` 定版，本项目唯一主线） |
> | **R2 各自量化再拼序列** | 两模态分别量化后拼接 / 堆叠 | ❌ **空 —— 这就是 V4-P** |
> | **R3 量化过程受跨模态监督** | 量化器本身被跨模态信号约束 | ❌ 空（V4-K 只填半格） |
> | **R4 协同信号入模** | 协同当一等公民 | 🟡 半占（共现只到融合层，未进量化器 / 编码器） |
>
> 🔴 **R0 为什么既替代不了 V4-P、也替代不了 V4-O**：`text` 臂**一次动了三个变量** ——
> 去掉图像 **+** 去掉融合层 **+** 去掉共现监督，所以 `text → gate` 的 **+32.5% / +41.2%** 是三者之和，
> **无法归因到「图像内容有没有被用」**。而 **V4-O 是单变量**（架构与训练目标全不变，只把图像**内容**换成噪声 / shuffle），
> **V4-P 也是单变量**（两个模态都保留，只换图文相遇的位置）。
> **已有一条线索在预警 V4-O**：`concat`（**加了图像但没加共现监督**）只 **+6.4% / +6.0%**，
> 而 `mlp` / `gate`（加图像 **+** 加共现监督）是 **+22% ~ +41%** → 增益主体可能来自**训练目标**而非图像本体。
> 这正是必须用 V4-O 把它一刀切开的原因。双域共现召回 R@10（`fusion_report.json`，n_query=3000）：
> I&S `text` 0.0830 → `concat` 0.0883 → `mlp` 0.1013 → `gate` 0.1100；
> VG `text` 0.1497 → `concat` 0.1587 → `mlp` 0.1917 → `gate` 0.2113。

→ 故：「`gate`（量化前融合）vs「按模态各自量化再拼序列」（**R2 / V4-P**）这个对照**确实是空的**（R0 已占，替代不了它），
而同题材论文全部选择了 R2 或其变体。**V4-P 由「对照臂」升级为「**第一优先级的对照实验**」**，
仍排在 **V4-O 之后**：V4-O 判「图有没有被用」（单变量），V4-P 判「该怎么用」（单变量）。

> ✅ **同时保留原判断：以上都不等于「`gate` 是错的」** —— 双域上 `gate` 都显著优于 `text`
> （I&S ICR 0.8383 → 0.9582、第 0 层死码 65.5% → 0%、共现召回 R@10 +32.5%；VG +41.2%）。
> R0 对照已经证明的是「**`gate` 这一层有用**」；**尚未证明的是「它比 R2（各自量化）更好用」** —— 后者才是 V4-P。
...
| 与 V0 血统一致（V0 用 `Qwen2.5-0.5B-**Instruct**`，见 `V0_MINIONEREC_TECH_DOC.md §sft_3090.sh 配置`） | ✅ 复刻参照更近 | **post-trained** |
