...
**优先级**：**V4-H 排第一**——它是唯一能回答「这 +40% 的代理指标提升，到底有没有用」的实验；
在它出结果之前，**不建议把白化直接接进正式 SID 生产链路**。
...
| **R1 早期融合** | **量化前**融成一个向量 | **PLUM SID-v2** (Google/YouTube, WWW'26)、**FusID** (2601.08764)、**MMQ** (阿里国际/Lazada, WSDM'26)、**OneMall** (快手电商, 2601.21770)、**Text-as-Vision** 早融合 | concat/门控/MLP/MoE 学一个联合向量，再送 RQ；**本项目现在的 `gate` 融合在这一族** |
...
| **R4 协同信号入模** | 把**行为信号**当一等公民接进 SID 链路 | **PLUM**（共现对比正则）、**OneRec**（I2I 对比 + caption loss）、**MSCGRec**（SASRec embedding **单独 RQ**）、**MMQ**（behavior-aware 微调）、**MMGRec**（GCN 进 encoder） | 四条不同强度：加一项损失 / 加一个辅助任务 / **当成第四个模态** / 让 CF 反向更新分词器（STE） |
...
**这三条对本项目的直接含义**：我们 V0' 纯文本锚点与 `gate` 融合在 I&S 上「打得有来有回」**符合文献规律**，
不是实现有问题；融合要挣到增益，靠的是**对齐质量**（§4.6.1/§4.6.3）与**协同信号注入**（R4），
而不是「把图像接上」这个动作本身。
...
|---|---|---|---|---|
| **V4-K** | **共现对比正则**：在 RQ-VAE 目标里加一项 item-item 共现对比（`L = L_recon + L_rq + λ·L_con`），不改结构、只改损失 | PLUM SID-v2（该项是消融里单项贡献最大的） | **极低**（一次 RQ-VAE 训练，I&S ≈ 68 min） | raw ICR ≥ 现（0.9582 / 0.9744）**且** 重建 R² 不降 **且** VG 的 L0 死码 ≤ 现（3.52%） |
